...
class MultimodalBertEncoder(nn.Module):
    ...
    def forward(self, input_txt, attention_mask, segment, input_img):
        ...
        img_tok = (torch.LongTensor(input_txt.size(0), self.args.num_image_embeds + 2).fill_(0).cuda())
...
class AttentionFusion(object):
    ...
    def __call__(self, text_embedding, knowledge_embedding):
        # concat_dim_zero = torch.cat([list_tensors], dim=1)
        attention_fused_tensors = self.attention_layer(text_embedding, knowledge_embedding, knowledge_embedding,
                                                       need_weights=False)
...
class MultimodalBertClf(nn.Module):
    def __init__(self, args):
        ...
        # self.clf = nn.Linear(args.hidden_sz, args.n_classes)
        self.linearlayer1 = nn.Linear(args.hidden_sz, 100)
        # torch.nn.init.xavier_uniform(self.linearlayer1.weight)
        self.dropout1 = nn.Dropout(0.5)
        self.act1 = nn.ReLU()
        self.linearlayer2 = nn.Linear(100, 100)
        # torch.nn.init.xavier_uniform(self.linearlayer2.weight)
        self.dropout2 = nn.Dropout(0.5)
        self.act2 = nn.ReLU()
        self.attention_layer = torch.nn.MultiheadAttention(100, 4)
        # torch.nn.init.xavier_uniform(self.attention_layer.weight)
        # self.layer_norm = nn.LayerNorm(256)
        self.dropout3 = nn.Dropout(0.5)
        self.act3 = nn.ReLU()
        self.pre_classifier = nn.Linear(100, 100)
        # torch.nn.init.xavier_uniform(self.pre_classifier.weight)
        self.dropout4 = nn.Dropout(0.5)
        self.clf = nn.Linear(100, args.n_classes)
    ...
    def forward(self, txt, mask, segment, img, concept_net_embeddings):
        ...
        x1 = self.linearlayer2(x1)
        x1 = self.act2(x1)
        x1 = self.dropout2(x1)
        # x = torch.cat([x, x1], dim=1)
        x = self.attention_layer(x, x1, x1, need_weights=False)
        x = self.dropout3(x[0])
...
class JsonlDataset(Dataset):
    ...
    def __getitem__(self, index):
        concept_net_embeddings = torch.from_numpy(np.array(self.data[index]["embeddings_ss"]).T).mean(axis=0)
        concept_net_embeddings = concept_net_embeddings.float()
        print(concept_net_embeddings)
...
def get_vocab(args):
    ...
    if args.model in ["bert", "mmbt", "concatbert"]:
        ...
        # bert_tokenizer = RobertaTokenizer.from_pretrained('roberta-base', truncation=True, do_lower_case=True)
...
# python train_MMBT_ViT_Bert.py --batch_sz 32 --img_hidden_sz 768 --gradient_acuumulation_steps 40 --gradient_accumulation_steps 40 --savedir test --name mmbt_model_run --data_path C:\Users\tpadhi1\Desktop\Adobe\mmbt-kg\data_prep_codes\Experiments\Transe --model mmbt --num_image_embeds 197 --freeze_txt 5 --freeze_img 3 --ma
