...
class MultimodalBertClf(nn.Module):
    def __init__(self, args):
        ...
        self.linearlayer3 = nn.Linear(256, 32)
        self.linearlayer2 = nn.Linear(32, 32)
        self.clf = nn.Linear(32, args.n_classes)

    def forward(self, txt, mask, segment, img):
        x = self.enc(txt, mask, segment, img)
        x = self.linearlayer1(x)
        x = self.linearlayer3(x)
        # print('x.shape after liner layer 1---->', x.shape)
...
class JsonlDataset(Dataset):
    ...
    def __getitem__(self, index):
        # concept_net_embeddings = torch.from_numpy(calculate_embeddings(self.data[index]["text"]))
...
# python train_MMBT_cuda.py --batch_sz 64 --gradient_accumulation_steps 40 --savedir results_mmbt_12_9/
# --name mmbt_model_run --data_path C:\Users\tpadhi1\Desktop\Adobe\mmbt-kg\kickstarter_dataset_processed
# --model mmbt --num_image_embeds 3 --freeze_txt 5 --freeze_img 3 --max_epochs 50
