...
class KGBertEmbeddings(nn.Module):
    def __init__(self, args, embeddings):
        super(KGBertEmbeddings, self).__init__()
        self.args = args
        self.kg_embeddings = nn.Linear(args.kg_embedding_dimension, 768)
    ...
    def forward(self, ccn_embeddings, token_type_ids):
        bsz = ccn_embeddings.shape[0]
        num_kg_embds = ccn_embeddings.shape[1]
        seq_length = num_kg_embds + 1  # +1 for sep token
        kg_embeds = self.kg_embeddings(ccn_embeddings)
        # get the separator token embeds
        sep_id = torch.LongTensor([self.args.vocab.stoi["[SEP]"]]).cuda()
        sep_id = sep_id.unsqueeze(0).expand(bsz, 1)
        sep_token_embeds = self.word_embeddings(sep_id)
        # built the bert embeddings in BERT style
        token_embeddings = torch.cat([kg_embeds, sep_token_embeds], dim=1)
...
class ImageBertEmbeddings(nn.Module):
    ...
    def forward(self, input_imgs, token_type_ids):
        ...
        token_embeddings = torch.cat([cls_token_embeds, imgs_embeddings, sep_token_embeds], dim=1)
        position_ids = torch.arange(seq_length, dtype=torch.long).cuda()
        position_ids = position_ids.unsqueeze(0).expand(bsz, seq_length)
        position_embeddings = self.position_embeddings(position_ids)
        token_type_embeddings = self.token_type_embeddings(token_type_ids)
...
class MultimodalBertEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.kg_embeddings = KGBertEmbeddings(args, self.txt_embeddings)
        self.vit_encoder = VisionTransformerEncoder(args)
        self.encoder = bert.encoder
        self.pooler = bert.pooler
        self.clf = nn.Linear(args.hidden_sz, args.n_classes)
        self.cross_attn_image_knowledge = nn.MultiheadAttention(768, 16, 0.1, batch_first=True)
        self.cross_attn_image_text = nn.MultiheadAttention(768, 16, 0.1, batch_first=True)
        self.cross_attn_text_knowledge = nn.MultiheadAttention(768, 16, 0.1, batch_first=True)
        self.cross_attn_text_image = nn.MultiheadAttention(768, 16, 0.1, batch_first=True)

    def forward(self, input_txt, attention_mask, segment, input_img, conceptnet_embeddings):
        bsz = input_txt.size(0)
        num_kg_tokens = conceptnet_embeddings.shape[1]  # how many concept net entities ??
        attention_mask_for_knowledge = torch.ones(bsz, num_kg_tokens + 1).long().cuda()  # +1 for the separator token
        attention_mask = torch.cat(
            [
                torch.ones(bsz, self.args.num_image_embeds + 2).long().cuda(),
                attention_mask,
                attention_mask_for_knowledge
            ],
            dim=1,
        )
        ...
        # num_kg_tokens is the parameter that tells us basically how many knowledge tokens do we have
        kg_tok = (
            torch.LongTensor(input_txt.size(0), num_kg_tokens + 1)
            .fill_(0)
            .cuda()
        )
        # kg_tok = (
        ...
        kg_embed_out = self.kg_embeddings(conceptnet_embeddings, kg_tok)
        txt_embed_out = self.txt_embeddings(input_txt, segment)

        # print(img_embed_out.shape, 'img_embed_out.shape')
        # print(kg_embed_out.shape, 'kg_embed_out.shape')
        # print(txt_embed_out.shape, 'txt_embed_out.shape')

        img_embed_out_ca_knowledge = self.cross_attn_image_knowledge(img_embed_out, kg_embed_out, kg_embed_out)[0]
        img_embed_out_ca_text = self.cross_attn_image_text(img_embed_out, txt_embed_out, txt_embed_out)[0]
        txt_embed_out_ca_knowledge = self.cross_attn_text_knowledge(txt_embed_out, kg_embed_out, kg_embed_out)[0]
        txt_embed_out_ca_image = self.cross_attn_text_knowledge(txt_embed_out, img_embed_out, img_embed_out)[0]

        img_embed_out = torch.mean(torch.stack([img_embed_out_ca_knowledge, img_embed_out_ca_text], dim=1), dim=1)
        txt_embed_out = torch.mean(torch.stack([txt_embed_out_ca_knowledge, txt_embed_out_ca_image], dim=1), dim=1)

        encoder_input = torch.cat([img_embed_out, txt_embed_out, kg_embed_out], 1)  # Bx(TEXT+IMG)xHID
...
class MultimodalBertClf(nn.Module):
    ...
    def forward(self, txt, mask, segment, img, conceptnet_embeddings):
        x = self.enc(txt, mask, segment, img, conceptnet_embeddings)
        x = self.dropout1(x)
...
class JsonlDataset(Dataset):
    def __init__(self, data_path, tokenizer, transforms, vocab, args):
        ...
        self.img_data_dir = r'C:\Users\tpadhi1\Desktop\Adobe\mmbt-kg\data_prep_codes\Experiments\Transe'
    ...
    def __getitem__(self, index):
        concept_net_embeddings = torch.from_numpy(np.array(self.data[index]["embeddings_ss"]).T)
        ...
        if self.args.model in ["img", "concatbow", "concatbert", "mmbt"]:
            try:
                if self.data[index]["img"]:
                    # print('the path', os.path.join(self.img_data_dir, self.data[index]["img"]))
                    image = Image.open(os.path.join(self.img_data_dir, self.data[index]["img"])).convert("RGB")
                    # print('Image exists, so path is working ')
                else:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--kg_embedding_dimension", type=int, default=256)
...
# python train_MMBT_ViT_Bert_Conceptnet_input_kg_fusion.py --batch_sz 32 --img_hidden_sz 768 --savedir test100 --name mmbt_model_run --data_path C:\Users\tpadhi1\Desktop\Adobe\mmbt-kg\data_prep_codes\Experiments\Transe\Transe_mhgrn --model mmbt --num_image_embeds 197 --kg_embedding_dimension 100
